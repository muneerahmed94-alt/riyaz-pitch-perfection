...
| Stability | Detection runs at ~60 estimates/sec, then passed through a 3-tap median filter (outlier rejection) and an adaptive exponential moving average, plus an octave-jump guard. The EMA tracks fast (α≈0.85) when the pitch clearly moves — so quick paltas/alankars don't lag — and smooths harder (α≈0.55) on steady notes to kill jitter. |
