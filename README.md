...
| Stability | Detection runs at ~60 estimates/sec, then passed through a 3-tap median filter (outlier rejection) and an exponential moving average (~25 ms time constant), plus an octave-jump guard. The median filter keeps the trail from twitching on one-off stray estimates while staying responsive to real pitch movement. |
