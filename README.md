...
- **Metronome (optional)** — a footer **Metronome** button starts/stops a
  ...
  - **Volume** — click loudness, level 1–10 via a slider and ± buttons
...
| Stability | Detection is throttled to ~30 estimates/sec (decoupled from the 60 Hz display refresh), then passed through a 3-tap median filter (outlier rejection) and an exponential moving average (~100 ms time constant), plus an octave-jump guard. This matches standard pitch-tracker practice and keeps the trail from twitching on every sub-frame vocal wobble. |
...
| Metronome (optional) | A look-ahead scheduler (`setInterval` tick + `AudioContext` sample-clock scheduling) queues short sine clicks — accented first beat, plain remaining beats — with tap-tempo averaging. |
