...
| Stability | Detection is throttled to ~45 estimates/sec (decoupled from the 60 Hz display refresh), then passed through a 3-tap median filter (outlier rejection) and an exponential moving average (~50 ms time constant), plus an octave-jump guard. This matches standard pitch-tracker practice and keeps the trail from twitching on every sub-frame vocal wobble. |
